...
{
  const Eigen::Vector3f pos = pose.block<3, 1>(0, 3).cast<float>();
  const Eigen::Matrix3f rot = pose.block<3, 3>(0, 0).cast<float>();
  rec.log(name, rerun::Transform3D(rerun::Vec3D(pos.data()), rerun::Mat3x3(rot.data())));
}